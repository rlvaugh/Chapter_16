"""Check that a collection on numeric counts follows Benford's Law."""
import sys

# example below is for Trump vs Clinton, Illinois, 2016 Presidental Election

def load_data(file_name):
    """Open a text file of numbers & return contents as a list."""
    while True:
        try:
            with open(file_name) as f: 
                lines = f.read().strip().split('\n') # loads as strings
                lines = [int(i) for i in lines] # turn strings to integers
                break
...
def steal_votes(opponent_votes, candidate_votes, scalar):
    ...
    for opp_vote, can_vote in zip(opponent_votes, candidate_votes):
        new_opp_vote = int(opp_vote * scalar)
